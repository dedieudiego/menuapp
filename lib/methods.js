...
import { Recipes, Ingredients, Categories, SubCategories, Measures, Difficulties, Users, Menus } from '../lib/collections';
 
Meteor.methods({
	firstTime(id) {
		const userUpdated = Meteor.users.update(id, { $set: { notFirstTime: true } });
		return userUpdated;
	},
	...
	saveMenu(menu, user) {
		let newMenu = Menus.insert({menu: menu, userId: user});
		return newMenu;
	}
});